...
Let's just say that the fractionation factor $\alpha$ for microbial sulfate reduction to sulfide is 0.4 per mil. 
$$
\alpha_{HS^- / SO_4^{2-}} = \frac{\delta^{34}S_{HS^-} + 1}{\delta^{34}S_{SO_4^{2-}} + 1} = 0.4
$$
We will also assume that sulfide in shallow groundwater has been reduced once, and sulfide in deep groundwater has been reduced, re-oxidized, and reduced again. Calculate the $\delta^{34}S$ value of both shallow and deep groundwater for each system.

```{r}
d34S_precip <- 5.0

d34S_awm_sgw <- (0.4 * (d34S_awm_surf + 1)) - 1
d34S_psl_sgw <- (0.4 * (d34S_psl_surf + 1)) - 1
d34S_saw_sgw <- (0.4 * (d34S_saw_surf + 1)) - 1

d34S_awm_dgw <- (0.4 * (d34S_awm_sgw + 1)) - 1
d34S_psl_dgw <- (0.4 * (d34S_psl_sgw + 1)) - 1
d34S_saw_dgw <- (0.4 * (d34S_saw_sgw + 1)) - 1
```
